...
} from '../lib/photograph-section-order'
...
export const onRequestPatch: PagesFunction<Env> = async (context) => {
    ...
    const orderUpdate = parsePhotographSectionOrderUpdate(body)
    if (!orderUpdate) {
        return Response.json(
            { error: '대주제 순서 변경값 형식이 올바르지 않습니다.' },
            { status: 400 },
        )
    }
    ...
    try {
        ...
        console.error('Photographs 대주제 순서 저장 실패:', error)
        return Response.json(
            { error: 'Photographs 대주제 순서를 저장하지 못했습니다.' },
            { status: 500 },
        )
    }
}
